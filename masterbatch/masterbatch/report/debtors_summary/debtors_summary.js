frappe.query_reports["Debtors Summary"] = {
	...
	onload: function (report) {
		report.page.add_inner_button(__("Export to Excel"), function () {
			...
			html += '<x:ExcelWorksheet><x:Name>Debtors Summary</x:Name>';
			...
			// Header row
			html += '<tr style="background-color:#1a3c5e;">';
			html += '<td style="font-weight:bold;color:#ffffff;padding:6px 10px;width:350pt;">Customer</td>';
			...
			html += '<td style="font-weight:bold;color:#ffffff;padding:6px 10px;width:100pt;text-align:right;">Received</td>';
			...
			data.forEach(function (row) {
				...
				var osColor = outstanding > 0 ? "#e67e22" : outstanding < 0 ? "#27ae60" : "#000000";

				if (isGroup) {
					var bgColor = indent === 0 ? "#dce6f1" : "#eaf2fb";
					var fontSize = indent === 0 ? "11pt" : "10.5pt";
					html += '<tr style="background-color:' + bgColor + ';">';
					html += '<td style="font-weight:bold;font-size:' + fontSize + ';padding:5px 8px;padding-left:' + padding + ';color:#1a3c5e;">' + label + '</td>';
					...
					html += '<td style="font-weight:bold;font-size:' + fontSize + ';padding:5px 8px;text-align:right;color:#2980b9;">' + formatCurrency(row.invoiced) + '</td>';
					html += '<td style="font-weight:bold;font-size:' + fontSize + ';padding:5px 8px;text-align:right;color:#27ae60;">' + formatCurrency(row.received) + '</td>';
					...
					html += '<td style="padding:4px 8px;text-align:right;color:#2980b9;">' + formatCurrency(row.invoiced) + '</td>';
					html += '<td style="padding:4px 8px;text-align:right;color:#27ae60;">' + formatCurrency(row.received) + '</td>';
			...
			a.href = url;
			a.download = "Debtors_Summary_" + as_on + ".xls";
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if (column.fieldname === "label") {
			if (!data.party_id) {
				...
				var url = "/app/query-report/General Ledger"
					...
					+ "&party_type=" + encodeURIComponent("Customer")
		...
		if (column.fieldname === "received" && value) {
		...
		if (column.fieldname === "outstanding") {
			...
			if (amt > 0) {
				value = "<span style='color:#e67e22;font-weight:700'>" + display + "</span>";
			} else if (amt < 0) {
